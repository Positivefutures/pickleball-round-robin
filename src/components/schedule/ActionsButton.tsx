 ...
 * come from the same image, which is a 375pt screen at 2.73x.
 */
...
function Halftone({ side, colour }: { side: 'left' | 'right'; colour: string }) {
  ...
  return (
    <span
      aria-hidden="true"
      className="pointer-events-none absolute top-1/2 hidden h-[74px] w-[150px] -translate-y-1/2 min-[380px]:block"
      style={{
...
export function ActionsButton({ onClick }: { onClick: () => void }) {
  return (
    // The four tiles hang 14px above the panel and <main> opens with only 16,
    ...
    <div className="no-print flex justify-center pt-[22px]">
      <div className="relative">
        ...
        <button
          ...
          className="relative flex h-[60px] w-[104px] flex-col items-center justify-end rounded-2xl
                     pb-2 text-white shadow-[0_4px_10px_rgba(0,0,0,0.18)] ring-2 ring-white
        ...
        >
          ...
          <span
            aria-hidden="true"
            className="absolute -top-3.5 grid grid-cols-2 gap-1"
          >
            {TILES.map((Icon, i) => (
              <span
                key={i}
                className="flex h-[21px] w-[21px] items-center justify-center rounded-[6px]
